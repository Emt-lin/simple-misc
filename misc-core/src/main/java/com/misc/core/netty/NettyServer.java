...
public final class NettyServer<ProtoInBound, ProtoOutBound, ChannelInBound, ChannelOutBound> implements NettyNode {
    ...
    public static abstract class Builder<ProtoInBound, ProtoOutBound, ChannelInBound, ChannelOutBound> {
        ...
        private void check() {
            ...
            server.bossGroup = server.bossGroup == null ? new NioEventLoopGroup(1,
                    new DefaultThreadFactory(NetUtils.formatAddr(server.address), true)) : server.bossGroup;

            server.workerGroup = server.workerGroup == null ? new NioEventLoopGroup(Constants.DEFAULT_IO_THREADS,
                    new DefaultThreadFactory(NetUtils.formatAddr(server.address), true)) : server.workerGroup;

            server.threadPool = new ThreadPool(this.threadPoolSize == 0 ? DEFAULT_THREAD_SIZE :
                    this.threadPoolSize, this.threadQueueSize == 0 ? DEFAULT_THREAD_QUEUE_SIZE :
                    threadQueueSize, StringUtils.isEmpty(this.threadName) ? NetUtils.formatAddr(server.address) : this.threadName);
