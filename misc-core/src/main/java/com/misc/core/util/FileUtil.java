...
public class FileUtil {
    ...
    public static final int LEN_1_KB = 1024 * 1;
    ...
    public static List<byte[]> cuttingFile(File fileName, int delimiter) throws IOException {
        ...
        try(RandomAccessFile file = new RandomAccessFile(fileName, "r");
            ...
            while (size > delimiter) {
                ...
                // 我们采用的是堆内存， 不是直接内存。原因：我们要做数组拷贝，没必要
     ...
     * @param fileName 文件路径
     * @param bytes 字节流
     * @throws IOException 中途异常
     */
    public static void mergingFile(File fileName, byte[] bytes) throws IOException {
        try(RandomAccessFile upload = new RandomAccessFile(fileName, "w");
...
//    };
     ...
     * 解压缩文件
     */
    public static boolean unCompressFile(File src, File dest, Function<byte[], byte[]> unCompressHandler)
        throws IOException {
     ...
     * 压缩文件
     */
    public static boolean compressFile(File src, File dest, Function<byte[], byte[]> compressHandler)
       throws IOException {
        if (dest != null && !dest.exists()) {
            boolean isOk = dest.createNewFile();
            if (!isOk) {
    ...
    /**
     * 读取一个文件的文件流. 也就是字节数组
     * 处理完这些字节数组. 然后再写入到新的文件中
     */
    public static boolean handlerFile(File src, File dest, int fileSize, Function<byte[], byte[]> handler)
        throws IOException {
